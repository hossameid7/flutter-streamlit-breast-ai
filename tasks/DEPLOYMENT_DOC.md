...
## 7a. Подтверждение запуска inference pipeline (Run / Logs Evidence)

Чтобы развертывание было подтверждено не только описанием, но и **фактическим запуском с логами**, добавлен smoke-тест инференс-конвейера: [`tests/deployment_smoke_test.py`](../tests/deployment_smoke_test.py).

Он проверяет три уровня контракта и пишет аудируемый лог в [`tests/deployment_smoke_test.log`](../tests/deployment_smoke_test.log):

1. **Input validation contract** — fail-fast проверки формата / разрешения (>=256x256) / дисперсии / каналов (те же правила, что и в Streamlit API и Flutter-клиенте до инференса).
2. **Tensor I/O contract** — проверка задокументированных форм тензоров TFLite: классификация `[1,224,224,3] -> [1,3]`, сегментация `[1,256,256,3] -> [1,256,256,1]`.
3. **Live forward pass** — если веса `.keras` присутствуют локально (они версионируются во внешнем хранилище согласно DoD из-за размера ~411 МБ), выполняется **реальный прямой проход** модели; иначе шаг честно помечается как SKIPPED.

**Результат последнего прогона:** все контракты выполнены, реальный forward pass модели классификации вернул тензор `(1, 3)` — см. `tests/deployment_smoke_test.log`. Запуск:

```bash
python tests/deployment_smoke_test.py
```

> **Примечание по весам:** файлы `.keras`/`.tflite` (~411 МБ) намеренно не коммитятся в Git (см. DoD, раздел «Артефакты»), а хранятся во внешнем артефакт-сторадже / DVC. Smoke-тест автоматически использует их, если они доступны локально, и не падает, если их нет.
